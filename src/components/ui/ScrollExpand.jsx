...
const ScrollExpand = ({
  ...
  const coordsRef = useRef({ startX: 0, startY: 0, endX: 0, endY: 0, hasPlaceholder: false });
  ...
  const applyProgress = useCallback(p => {
    ...
    if (titleRef.current) {
      ...
      if (coords.hasPlaceholder) {
        const x = coords.startX + (coords.endX - coords.startX) * e;
        const y = coords.startY + (coords.endY - coords.startY) * e;
        const scale = 1.22 + (1.0 - 1.22) * e;
        titleRef.current.style.transform = `translate3d(${x}px, ${y}px, 0) scale(${scale})`;
      ...
      if (titleEl) {
        ...
        if (placeholder) {
          ...
          coordsRef.current = {
            startX,
            startY,
            endX: pRect.left - stageRect.left,
            endY: pRect.top - stageRect.top,
            hasPlaceholder: true
          };
        } else {
